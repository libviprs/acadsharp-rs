...
//! calls the script.
//!
//! This is the only part of H2.5 that can be proven from inside this repo. The
//! `main` arm of the rule cannot be reached by pushing a branch (you would have
//! to merge to get there), and the job's cross-repo half cannot do real work at
//! all until `acadsharp-rs-tests#1` gives the suite its
//! `acadsharp-rs = { path = "../acadsharp-rs" }` dependency back to this crate.
//! So the branch logic lives in a script, and the script is tested here.

use std::path::PathBuf;
use std::process::Command;

/// Message payload only. The script never touches the network, so this is just
/// a realistic 40-hex string to look for in the output.
const SHA: &str = "76e92ed464877f460917b2b80d8cd2f5ba5ca5cc";
...
fn the_suite_job_is_wired_the_way_the_rule_needs() {
    ...
    // The blocker has to be named where somebody reading a red job will see it.
    assert!(
        job.contains("acadsharp-rs-tests#1"),
        "the path-dependency refusal does not name its blocker:\n{job}"
    );
