...
//!
//! With no archive the crate still builds, checks, tests and documents: the
//! public surface is the same either way, and [`Decoder::new`] answers
//! [`Error::Unlinked`] instead of disappearing. A surface that changed shape
//! with an environment variable would be one a consumer could not write a
//! `cfg` for. [`batch`] never links at all: it reads bytes.
//!
